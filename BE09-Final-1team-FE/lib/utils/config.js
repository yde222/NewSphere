...
export const config = {
  ...
  api: {
    ...
    newsletterServiceUrl: process.env.NEXT_PUBLIC_NEWSLETTER_SERVICE_URL || 'http://localhost:8085', // 뉴스레터 서비스
...
export const getNewsServiceUrl = (endpoint) => {
  try {
    ...
    console.error('❌ Invalid URL generated:', { baseUrl: config.api.newsServiceUrl, endpoint, error });
    throw new Error(`Failed to construct URL: ${error.message}`);
  }
};

// 뉴스레터 서비스 URL 생성 헬퍼 함수
export const getNewsletterServiceUrl = (endpoint) => {
  const baseUrl = config.api.newsletterServiceUrl.replace(/\/$/, ''); // 끝의 슬래시 제거
  const cleanEndpoint = endpoint.replace(/^\//, ''); // 시작의 슬래시 제거
  return `${baseUrl}/${cleanEndpoint}`;
};
