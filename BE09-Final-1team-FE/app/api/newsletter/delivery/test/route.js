...
export async function POST(request) {
  try {
    console.log('뉴스레터 발송 테스트 API 호출');
    
    // 백엔드 API로 발송 테스트 요청
    const backendUrl = getNewsletterServiceUrl('/api/newsletter/delivery/test');
    ...
    console.log('백엔드 뉴스레터 발송 테스트 응답:', {
    ...
    return NextResponse.json({
      success: true,
      data: backendData.data,
      message: backendData.message || '뉴스레터 발송 테스트가 완료되었습니다.'
    });

  } catch (error) {
    console.error('뉴스레터 발송 테스트 서버 에러:', {
      error: error.message || '알 수 없는 오류',
      name: error.name,
      code: error.code,
      stack: error.stack,
      url: '/api/newsletter/delivery/test',
