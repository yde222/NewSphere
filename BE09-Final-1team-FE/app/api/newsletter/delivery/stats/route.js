...
export async function GET(request) {
  try {
    console.log('뉴스레터 발송 통계 조회 API 호출');
    
    // 백엔드 API로 발송 통계 조회 요청
    const backendUrl = getNewsletterServiceUrl('/api/newsletter/delivery/stats');
    ...
    console.log('백엔드 뉴스레터 발송 통계 조회 응답:', {
    ...
    return NextResponse.json({
      success: true,
      data: backendData.data,
      message: backendData.message || '발송 통계를 조회했습니다.'
    });

  } catch (error) {
    console.error('뉴스레터 발송 통계 조회 서버 에러:', {
      error: error.message || '알 수 없는 오류',
      name: error.name,
      code: error.code,
      stack: error.stack,
      url: '/api/newsletter/delivery/stats',
