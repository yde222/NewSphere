...
export async function POST(request) {
  try {
    console.log('뉴스레터 예약 발송 API 호출');
    
    const body = await request.json();
    console.log('예약 발송 요청 데이터:', body);
    
    // 백엔드 API로 예약 발송 요청
    const backendUrl = getNewsletterServiceUrl('/api/newsletter/delivery/schedule');
    ...
    return NextResponse.json({
      success: true,
      data: backendData.data,
      message: backendData.message || '뉴스레터가 예약되었습니다.'
    });

  } catch (error) {
    console.error('뉴스레터 예약 발송 서버 에러:', {
      error: error.message || '알 수 없는 오류',
      name: error.name,
      code: error.code,
      stack: error.stack,
      url: '/api/newsletter/delivery/schedule',
